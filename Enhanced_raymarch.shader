            ...
            pout frag(v2f i)
            {
               ...
               // float pixelRadius = 0.5;
                ...
                for (int i = 0; i < MAX_ITERATIONS; ++i) { //レイマーチングのループを実行
                    p = ro + rd * t;
                    float signedRadius = functionSign * dist(p);
                    ...
                    d = dist(p);
                    candidate_t += d;
                    if ( candidate_t > t_max)break;//レイが遠くに行き過ぎたか衝突した場合ループを終える
                    */
                }
                
                fixed4 col = float4(0,0,0,1);
                if (abs(candidate_t) > t_max){//candidate_t > t_max) { //レイが衝突していないと判断すれば描画しない
                    discard;
                }
                else {
